...
public class LecturerService {
    ...
    public LecturerEntity create(CreateLecturerRequest request, UserPrincipal principal) {
        ...
        if (request.email() != null && !request.email().matches("^[a-zA-Z0-9._%+-]+@(fpt\\.edu\\.vn|fe\\.edu\\.vn)$")) {
            throw ApiException.badRequest("Email must be from @fpt.edu.vn or @fe.edu.vn domain");
        }

        // 2. Check Uniqueness
        if (lecturerRepository.existsByEmail(request.email())) {
            throw ApiException.conflict("Email '" + request.email() + "' is already registered for another lecturer.");
        }
        ...
        user.setJiraAccountId("NOT_SET_" + account); // Lecturers don't have Jira; use unique pseudo-ID to satisfy DB
                                                     // unique constraint
    ...
    public LecturerEntity update(Integer id, UpdateLecturerRequest request, UserPrincipal principal) {
        ...
        // 2. Check Uniqueness (if email changed)
        if (request.email() != null && !request.email().equals(entity.getEmail())) {
            if (lecturerRepository.existsByEmailAndIdNot(request.email(), id)) {
                throw ApiException.conflict("Email '" + request.email() + "' is already used by another lecturer.");
            }
        }
    ...
    public void delete(Integer id, UserPrincipal principal) {
        ...
        if (!classRepository.findByLecturerId(id).isEmpty()) {
            throw ApiException
                    .badRequest("Cannot delete lecturer: they are still assigned to classes. Unassign them first.");
        }
        if (!groupRepository.findByLecturerId(id).isEmpty()) {
            throw ApiException
                    .badRequest("Cannot delete lecturer: they are still supervising groups. Unassign them first.");
        }
